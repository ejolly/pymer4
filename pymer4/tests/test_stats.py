...
def test_perm_test():
    ...
    y = np.random.normal(loc=2.5, size=15)
    result = perm_test(x, y, stat='tstat', n_perm=500,
                       n_jobs=1, return_dist=True)
    assert(len(result) == 3)
    y = np.random.normal(loc=2.5, size=10)
    result = perm_test(x, y, stat='tstat-paired',
                       n_perm=500, n_jobs=1, return_dist=False)
    assert(len(result) == 2)
    result = perm_test(x, y, stat='cohensd', n_perm=500,
                       n_jobs=1, return_dist=False)
    assert(len(result) == 2)
    result = perm_test(x, y, stat='pearsonr', n_perm=500,
                       n_jobs=1, return_dist=False)
    assert(len(result) == 2)
    result = perm_test(x, y, stat='spearmanr', n_perm=500,
                       n_jobs=1, return_dist=False)
    assert(len(result) == 2)
...
def test_tost():
    np.random.seed(10)
    lower, upper = -.1, .1
    x, y = np.random.normal(.145, .025, 35), np.random.normal(.16, .05, 17)
    ...
    np.random.seed(1)
    x, y = np.random.normal(.12, .025, 35), np.random.normal(.14, .05, 17)
    result = tost_equivalence(x, y, lower, upper)
    assert(result['In_Equivalence_Range'] is True)
    assert(result['Means_Are_Different'] is True)
    x, y = np.random.normal(.32, .025, 35), np.random.normal(.14, .05, 17)
    result = tost_equivalence(x, y, lower, upper)
