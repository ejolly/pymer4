...
def simulate_lm(num_obs, num_coef, coef_vals=None, corrs=None, mus=0.0, sigmas=1.0, noise_params=(0, 1), family='gaussian', seed=None):
    ...
    if coef_vals is not None:
        if len(coef_vals) - num_coef == 0:
            ...
            assert len(coef_vals) == num_coef + \
                1, "Number of coefficient values should be num_coef + 1 (for intercept)"
    ...
    assert isinstance(noise_params, tuple) and len(
        noise_params) == 2, "noise_params should be a tuple of (mean,std)"

    # Generate random design matrix
    if corrs is not None:
        X = easy_multivariate_normal(
            num_obs, num_coef, corrs, mus, sigmas, seed)
    else:
    ...
    dat = pd.DataFrame(np.column_stack([Y, X[:, 1:]]), columns=[
                       'DV'] + ['IV' + str(elem + 1) for elem in range(X.shape[1] - 1)])
...
def simulate_lmm(num_obs, num_coef, num_grps, coef_vals=None, corrs=None, grp_sigmas=.25, mus=0.0, sigmas=1.0, noise_params=(0, 1), family='gaussian', seed=None):
    ...
    assert isinstance(noise_params, tuple) and len(
        noise_params) == 2, "noise_params should be a tuple of (mean,std)"
    assert isinstance(grp_sigmas, int) or isinstance(grp_sigmas, list) or isinstance(
        grp_sigmas, float), "grp_sigmas should be scalar value or list"
    if not isinstance(grp_sigmas, list):
        grp_sigmas = [grp_sigmas] * (num_coef + 1)
    else:
        assert len(grp_sigmas) == len(
            b), "The length of a list of grp_sigmas must match the num_coef plus intercept!"
    ...
    blups = np.array([np.random.normal(est, sigma, num_grps)
                      for est, sigma in zip(b, grp_sigmas)]).T
    ...
    for grp in range(blups.shape[0]):
        ...
        y = np.dot(x, blups[grp, :]) + \
            np.random.normal(*noise_params, size=num_obs)
        if family == 'binomial':
            y = discrete_inverse_logit(y)
        if grp == 0:
            x_all, y_all = x, y
    ...
    grp_ids = np.array(
        [[elem] * num_obs for elem in range(1, num_grps + 1)]).ravel()

    data = pd.DataFrame(np.column_stack([y_all, x_all[:, 1:], grp_ids]), columns=[
                        'DV'] + ['IV' + str(elem + 1) for elem in range(x_all.shape[1] - 1)] + ['Group'])
    blups = pd.DataFrame(blups, columns=['Intercept'] + ['IV' + str(elem + 1) for elem in range(
        x_all.shape[1] - 1)], index=['Grp' + str(elem + 1) for elem in range(num_grps)])
    return data, blups, b
